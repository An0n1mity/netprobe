...
def generate_device_report(host, output_dir, menu_file_name="menu.html"):
    ...
    mac_address = host.get('MAC', 'Non renseignée')
    ip_address = host.get('IP', 'Non renseignée')
    hostname = host.get('HOSTNAME', 'Non renseigné')
    vendor = host.get('VENDOR', 'Non renseigné')
    first_seen = host.get('FIRST SEEN', 'Non renseignée')
    ...
    <body>
        <h1>Détails de l'appareil ({mac_address})</h1>
        <table>
            <tr><th>Adresse MAC</th><td>{mac_address}</td></tr>
            <tr><th>Adresse IP</th><td>{ip_address}</td></tr>
            <tr><th>Nom de domaine</th><td>{hostname}</td></tr>
...
def generate_menu_report(json_file, output_dir):
    ...
    for host in data:
        ...
        vendor = host.get("VENDOR", None)
        
        # Construire le display_name dynamiquement
        display_name_parts = [part for part in [mac, ip, hostname, vendor] if part]  # Exclut les valeurs `None` ou vides
    ...
    for host in data:
        ...
        # Chaque ligne du tableau est construite avec des valeurs explicites pour chaque colonne
        table_rows += f"<tr onclick=\"window.location.href='device_{mac.replace(':', '')}.html';\" style=\"cursor: pointer;\">"
        table_rows += f"<td>{mac}</td><td>{ip}</td><td>{hostname}</td><td>{vendor}</td></tr>"
...
if __name__ == "__main__":
    ...
    print(f"Generating report, please wait. It can take some time...")
